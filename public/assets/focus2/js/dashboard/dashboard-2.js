  ...
  $.ajax({
      ...
      success: function (response) {
          ...
          response.data.forEach((item) => {
              // Hanya menambahkan data jika ada nilai
              if (item.month && (item.settlement || item.pending || item.cancel)) {
          ...
          // Jika tidak ada data, tidak menampilkan chart
          if (months.length === 0 || paid.length === 0) {
          ...
          // Tambahkan label dan series hanya jika nilai ada
          if (totalPaid > 0) {
              pieData.labels.push("settlement");
              pieData.series.push({
                  value: totalPaid,
          ...
          if (pieData.labels.length === 0) {
              console.log('Tidak ada data untuk pie chart');
              return;
          }
